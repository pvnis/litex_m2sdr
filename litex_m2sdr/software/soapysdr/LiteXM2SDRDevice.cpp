...
void SoapyLiteXM2SDR::setSampleRate(
    ...
    if (direction == SOAPY_SDR_TX) {
        ...
        /* Calculate nanoseconds per sample in Q8.24 fixed-point format (integer-only math) */
        /* ns_per_sample_q24 = (1e9 * 2^24) / samplerate */
        _tx_stream.ns_per_sample_q24 = static_cast<uint64_t>((1e9 * (1ULL << 24)) / rate);
    }
...
long long SoapyLiteXM2SDR::getRFICTime() const {
    ...
    /* Latch the 64-bit Time (ns) by pulsing READ bit of TimeGenerator Control Register. */
    ...
    /* Read the upper/lower 32 bits of the 64-bit Time (ns) from TimeGenerator. */
    ...
    SoapySDR::logf(SOAPY_SDR_DEBUG, "Hardware TimeGenerator time (ns): %lld", (long long)time_ns);
