...
/*
 * TX Pipeline delay compensation (nanoseconds)
 * This is the delay between when a sample is written to DMA and when it's transmitted by AD9361.
 * Includes: DMA latency + CDC + FIFO + bit mode conversion + PHY serialization
 *
 * IMPORTANT: This value needs to be calibrated for your specific system.
 * Calibration procedure:
 *   1. Set up RX loopback or external measurement
 *   2. Transmit a known signal at a scheduled time
 *   3. Measure the actual TX time vs scheduled time
 *   4. Adjust this constant to compensate for the measured delay
 *
 * Typical range: 300-1000ns depending on clock domains and FIFO depths
 * Initial estimate: ~500ns (will need calibration)
 */
#define TX_PIPELINE_DELAY_NS  500
 ...
 **************************************************************************************************/
  private:
    ...
    struct TXStream: Stream {
        ...
        // timestamp management (integer-only to avoid accumulation errors)
        bool timestamp_mode;
        uint64_t sample_count;
        uint64_t base_timestamp;
        uint64_t next_timestamp;
        uint64_t ns_per_sample_q24;  // Nanoseconds per sample in Q8.24 fixed-point format
        uint64_t accumulated_ns;     // Direct nanosecond accumulator
