...
int m2sdr_open(struct m2sdr_dev **dev_out, const char *device_identifier)
{
    ...
    {
        const char *pci_addr = device_identifier;
        int poll_mode = M2SDR_POLL_ADAPTIVE;

        /* Accept "vfio:0000:01:00.0" or bare "0000:01:00.0" */
        if (!strncmp(pci_addr, "vfio:", 5))
            pci_addr += 5;

        if (!pci_addr || !pci_addr[0])
            pci_addr = "0000:01:00.0";

        dev->transport = M2SDR_TRANSPORT_VFIO;
        snprintf(dev->vfio_pci_addr, sizeof(dev->vfio_pci_addr), "%s", pci_addr);

        dev->vfio = m2sdr_vfio_open(pci_addr, poll_mode);
        if (!dev->vfio) {
            free(dev);
            return M2SDR_ERR_IO;
        }
